...
const { maybeCreateAccount, accountExists, accountHasTheKey, verifyAccount } = require('../../lib/near-helpers');


class InitNEARTestContracts {
    static async execute(command) {
        let keyStore = new nearlib.keyStores.InMemoryKeyStore();
        await keyStore.setKey(command.nearNetworkId, command.nearMasterAccount, nearlib.KeyPair.fromString(command.nearMasterSk));
