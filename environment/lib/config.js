...
class RainbowConfig {
    ...
    static declareOption(name, description, defaultValue='', noConfig=false) {
        this.paramDeclarations[name] = {description: description, defaultValue: defaultValue, noConfig: noConfig};
    }
    ...
    static addOptions(action, options) {
        ...
        for (let option of options) {
            let declaration = this.paramDeclarations[option];
            let paramCase = changeCase.paramCase(option);
            let snakeCase = changeCase.snakeCase(option);
    ...
    static _processArg(name, value, previous) {
        let constantCase = changeCase.constantCase(name);
        let camelCase = changeCase.camelCase(name);
        if (this.paramValues[name]) {
            console.error(`Argument ${name} is specified more than once.`);
            process.exit(1);
        }
        if (process.env[constantCase]) {
            this.paramValues[name] = {value: process.env[constantCase], paramType: 'env'};
        } else if (value) {
            this.paramValues[name] = {value: value, paramType: 'arg'};
        } else if (this.configFile.has(camelCase)) {
            this.paramValues[name] = {value: this.configFile.get(camelCase), paramType: 'config'};
        } else if (previous) {
    ...
    static getArgsNoDaemon() {
        ...
        for (let name in this.paramValues) {
            let value = this.paramValues[name];
    ...
    static saveConfig() {
        for (let name in this.paramValues) {
            ...
            if (value.paramType === 'arg' || value.paramType === 'default' || value.paramType === 'config') {
                let camelCase = changeCase.camelCase(name);
