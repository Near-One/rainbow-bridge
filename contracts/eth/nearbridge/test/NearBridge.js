...
describe('NearBridge', function () {
    ...
    it('should be ok', async function () {
        ...
        await expect(bridge.addLightClientBlock(block121998)).to.be.revertedWith('NearBridge: Epoch id of the block is not valid');
    ...
    if (process.env.NEAR_HEADERS_DIR) {
        it('ok with many block headers', async function () {
            ...
            await bridge.initWithValidators(borshifyInitialValidators(firstBlock.next_bps));
            await bridge.initWithBlock(firstBlockBorsh);
            await bridge.blockHashes(firstBlock.inner_lite.height);
            expect(await bridge.blockHashes(firstBlock.inner_lite.height)).to.be.a('string');
            ...
            for (let i = 1; i < blockFiles.length; i++) {
                ...
                await bridge.addLightClientBlock(blockBorsh);
                await bridge.blockHashes(block.inner_lite.height);
                await time.increase(3600);
                ...
                if (i >= 600) {
                    ...
                    for (let j = 0; j < block.approvals_after_next.length; j++) {
                        ...
                        if (block.approvals_after_next[j]) {
                            ...
                            expect(await bridge.checkBlockProducerSignatureInHead(j)).to.be.true;
                        }
                    }
                }
            }
