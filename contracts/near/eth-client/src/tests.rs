...
#[derive(Deserialize)]
struct AddBlockHeaderData {
    dag_nodes: Vec<DoubleNodeWithMerkleProof>,
    first_header: Vec<u8>,
    block_header: Vec<u8>,
}

fn read_add_block_header_data() -> AddBlockHeaderData {
    serde_json::from_reader(
        std::fs::File::open(std::path::Path::new(
            "./src/data/check_add_block_header.json",
        ))
        .unwrap(),
    )
    .unwrap()
...
#[test]
fn check_add_block_header() {
    testing_env!(get_context(vec![], false));

    let AddBlockHeaderData {
        dag_nodes,
        block_header,
        first_header,
    } = read_add_block_header_data();
    ...
    client.add_block_header(block_header, dag_nodes);
}
