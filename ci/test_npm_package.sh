...
if [ -n "${LOCAL_CORE_SRC+x}" ]; then
  rbbr prepare --core-src "$LOCAL_CORE_SRC"
else
  rbbr prepare
fi
rbbr start near-node
rbbr start ganache
...
rbbr init-near-contracts
rbbr init-eth-ed25519
# Use short lockup time for tests
rbbr init-eth-client --eth-client-lock-eth-amount 1e18 --eth-client-lock-duration 30
rbbr init-eth-prover
rbbr init-eth-erc20
...
rbbr transfer-eth-erc20-from-near --amount 1 --near-sender-account rainbow_bridge_eth_on_near_prover \
