...
''')
    ...
    parser.add_argument(
        '--home',
        help="If specified, uses this directory instead of ~ to for .rainbowup files.",
        default=os.path.expanduser(f'~/.rainbowup'))
    parser.add_argument(
        '--source',
        help="If specified, will use Rainbow bridge source in that folder. Otherwise "
        "will use `~/.rainbowup/bridge`. If source does not exist it will "
        "download it from github.")
    parser.add_argument(
        '--nearcore_source',
        help="If specified, will use nearcore source in that folder. Otherwise "
        "will use `~/.rainbowup/core`. If source does not exist it will "
        "download it from github.")
    parser.add_argument(
        '--eth_node_url',
        help='If specified will use external Ethereum node instead of starting'
        'Ganache')
    parser.add_argument(
        '--near_node_url',
        help='If specified, will not start local Near node and will connect to '
  ...
  def _near_master_sk(self):
    if self._is_external_node():
      ...
      with open(
          os.path.join(self._near_datafolder(), 'node0/validator_key.json'),
          'r') as f:
        return json.load(f)['secret_key']
  ...
  def _test(self):
    ...
    contract_address = next(
        (s for s in stdout.decode("utf-8").split("\n") if s.startswith("0x")),
        None)
    p = subprocess.Popen([
        'yarn', 'run', 'oz', 'send-tx', '--to', contract_address, '--method',
        '"emitEvent(uint256,uint256,uint256)"', '--args', '1,2,3', '--network',
        'development', '--no-interactive'
    ],
