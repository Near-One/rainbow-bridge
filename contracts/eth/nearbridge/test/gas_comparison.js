...
describe("Test gas required by calls through proxy", () => {
  ...
  it("should add block with Bridge deployed as proxy", async function () {
    ...
    let tx = await NearBridgeAsProxy.addLightClientBlock(block9610);
...
describe("Test gas required by calls without proxy", () => {
  ...
  it("should add block with Bridge deployed without proxy", async function () {
    const block9605 = borshify(require("./block_9605.json"));
    const block9610 = borshify(require("./block_9610.json"));
    ...
    let tx = await NearBridge.addLightClientBlock(block9610);
