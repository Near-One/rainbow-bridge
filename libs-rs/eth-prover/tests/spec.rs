...
fn block_hash_safe_from_eth_client() {
    ...
    let res =  near.function_call(&mut r, "eth-prover", "assert_ethbridge_hash", &assert_ethbridge_hash_args.try_to_vec().unwrap(),ntoy(0)).unwrap();
