...
class EthProverTesterService:
  ...
  def run(self):
    env = dict(
        ...
        ETH_CLIENT_CONTRACT_PATH=os.path.join(self.args.source,
                                              'libs-rs/res/eth_client.wasm'),
        ETH_PROVER_ACC_ID="ethprover",
        ETH_PROVER_SK=self.bridge_sk,
        ETH_PROVER_INIT_BALANCE="1000000000000000000000000000",
        ETH_PROVER_CONTRACT_PATH=os.path.join(self.args.source,
                                              'libs-rs/res/eth_prover.wasm'),
    ...
    if self.args.rainbow_environment_image:
      ...
      env = {**os.environ, **env}
      return subprocess.Popen(['node', 'index.js', 'start', 'test_ethprover'],
                              env=env,
