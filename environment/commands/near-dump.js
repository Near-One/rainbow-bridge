...
class NearDump {
    ...
    static async dumpProofs(nearNodeUrl, path, latestBlock, numBlocks) {
        console.log(`Downloading ${numBlocks} light client proofs start from ${latestBlock.header.height}`);
        ...
        while (numBlocks > 0) {
            ...
            if (newLatestBlock.header.height > latestBlock.header.height) {
                ...
                console.log(`There are ${chunk.transactions.length} txns in block ${latestBlock.header.height}'s chunk`);
                console.log(`There are ${chunk.receipts.length} receipts in block  ${latestBlock.header.height}'s chunk`);
                ...
                for (let i in chunk.receipts) {
                    let proof = await getReceiptProof(nearNodeUrl, newLatestBlock, chunk.receipts[i]);
                    await NearDump.saveProof(latestBlock.header.height, 'receipt', i, proof, path);
                }
                latestBlock = newLatestBlock;
                numBlocks--;          
            } else {
