...
class Setup {
    ...
    async setup() {
        ...
        // this.masterAccountPK = nearlib.KeyPair.fromString(this.masterAccountSK).publicKey;
    ...
    async maybeCreateAccount(accountId, accountPK, initBalance, contractPath) {
        if (!await this.accountExists(accountId)) {
            ...
            try {
                await masterAccount.createAccount(accountId, accountPK, new BN(initBalance));
            } catch (e) {
